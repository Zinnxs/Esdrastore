      ...
      <section className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-glow sm:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black text-slate-950 sm:text-5xl">Pedidos realizados</h1>
          </div>
          <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm font-medium text-slate-600">
            {orders.length} pedido(s) finalizado(s)
          </div>
        </div>
        ...
        {orders.length > 0 ? (
          ...
          <div className="mt-6 rounded-[28px] border border-dashed border-slate-300 bg-slate-50 p-10 text-center">
            <p className="text-xl font-bold text-slate-900">Nenhum pedido ainda.</p>
            <p className="mt-2 text-slate-500">Finalize uma compra no checkout para preencher esta tabela automaticamente.</p>
          </div>
        )}
      </section>
    </div>
